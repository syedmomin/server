...
const collection = {
  create: async function (req, res) {
    try {
      ...
      await db.query(
        "SELECT COUNT(*) AS count FROM cufftype WHERE name = ?",
        [name],
        (error, results) => {
          ...
          if (count > 0) {
            ...
            db.query(
              "INSERT INTO cufftype (shortName, name,status ) VALUES (?,?,?)",
              [shortName, name, 1],
              (error, results) => {
                if (error) {
                  res.status(500).send({
  ...
  update: async function (req, res) {
    try {
      ...
      const updateSql = `UPDATE cufftype SET ${Object.keys(updateColumns)
        .map((key) => `${key} = ?`)
        .join(", ")} WHERE id = ?`;
  ...
  getAll: async function (req, res) {
    try {
      await db.query("SELECT * FROM cufftype", async (error, results) => {
        if (error) {
          res.status(500).send({
            code: 500,
            status: false,
