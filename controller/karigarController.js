...
const collection = {
  create: async function (req, res) {
    try {
      const { karigar_image, name, phone, cnic, address } = req.body;
      await db.query(
        ...
        (error, results) => {
          ...
          if (count > 0) {
            ...
            db.query(
              "INSERT INTO karigar (karigar_image,name, phone, cnic,address,status ) VALUES (?,?,?,?,?,?)",
              [karigar_image, name, phone, cnic, address, 0],
              (error, results) => {
                if (error) {
                  res.status(500).send({
                    code: 500,
