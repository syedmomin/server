...
const collection = {
  create: async function (req, res) {
    try {
      ...
      await db.query(
        "SELECT COUNT(*) AS count FROM colortype WHERE name = ?",
        [name],
        (error, results) => {
          ...
          if (count > 0) {
            ...
            db.query(
              "INSERT INTO colortype (shortName, name,status ) VALUES (?,?,?)",
              [shortName, name, 1],
              (error, results) => {
                if (error) {
                  res.status(500).send({
  ...
  update: async function (req, res) {
    try {
      ...
      const updateSql = `UPDATE colortype SET ${Object.keys(updateColumns)
        .map((key) => `${key} = ?`)
        .join(", ")} WHERE id = ?`;
  ...
  delete: async function (req, res) {
    try {
      await db.query(
        "DELETE FROM colortype WHERE id = ?",
        [req.body.id],
        async (error, results) => {
          if (error) {
            res.status(500).send({
  ...
  getAll: async function (req, res) {
    try {
      await db.query("SELECT * FROM colortype", async (error, results) => {
        if (error) {
          res.status(500).send({
            code: 500,
            status: false,
  ...
  getById: async function (req, res) {
    try {
      await db.query(
        "SELECT * FROM colortype WHERE id = ?",
        [req.body.id],
        async (error, results) => {
          if (error) {
            res.status(500).send({
