...
const collection = {
  ...
  getAll: async function (req, res) {
    try {
      await db.query(
        "SELECT * FROM order_master ORDER BY id DESC",
        async (error, results) => {
          if (error) {
            res.status(500).send({
              code: 500,
